...
if [ -z "$1" ] && [ -z "$2" ]; then
  ...
  echo "Valid node types are memory, ebs, sharedmemory, benchmark, and routing."
...
if [ "$1" = "sharedmemory" ]; then
  ...
  echo "Unrecognized node type $1. Valid node types are memory, EBS, sharedmemory, benchmark, and routing."
fi
