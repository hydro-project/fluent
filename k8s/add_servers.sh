...
if [ -z "$1" ] && [ -z "$2" ]; then
  ...
  echo "Valid node types are memory, ebs, shared-memory, benchmark, and routing."
...
if [ "$1" = "shared-memory" ]; then
  ...
  echo "Unrecognized node type $1. Valid node types are memory, EBS, shared-memory, benchmark, and routing."
fi
