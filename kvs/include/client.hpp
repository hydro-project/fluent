...
class KvsClient {
 public:
  KvsClient(std::string elb_addr, unsigned routing_thread_count, std::string ip, unsigned tid=0, unsigned timeout=10000) :
    ip_(ip), 
    timeout_(timeout),
    routing_thread_count_(routing_thread_count)
  {
    std::vector<std::string> addrs { elb_addr };
    Init(addrs, routing_threads, ip, tid);
    
    local_ = false;
  }
  
  KvsClient(std::vector<Address> addrs, unsigned routing_thread_count, std::string ip, unsigned tid=0, unsigned timeout=10000) :
    route_addrs_(addrs),
    ip_(ip),
    timeout_(timeout),
    routing_thread_count_(routing_thread_count)
  {
    ...
    socket_cache = SocketCache(&context, ZMQ_PUSH);

    std::hash<std::string> hasher;
    seed_ = time(NULL);
    seed_ += hasher(ip);
    seed_ += tid;
    logger_->info("Random seed is {}.", seed);

    // bind the two sockets we listen on
    key_address_puller_ = zmq::socket_t(&context, ZMQ_PULL);
    key_address_puller_.bind(self.get_key_address_bind_addr());
    key_address_puller.setsockopt(ZMQ_RCVTIMEO, &timeout_, sizeof(timeout_));
    
    response_puller_ = zmq::socket_t(&context, ZMQ_PULL);
    response_puller_.bind(self.get_request_pull_bind_addr());
    response_puller.setsockopt(ZMQ_RCVTIMEO, &timeout_, sizeof(timeout_));
 ...
 public:
  ...
  std::string get(Key key, unsigned trial_limit) {
    ...
    request.set_requets_type(get_request_type("GET")); 
    ...
    KeyTuple tuple = response.tuple(0);
    if (tuple.error() == 1) {
      logger->info("Key {} does not exist and could not be retrieved.", key);
      return NULL;
    }
   ...
   * use a default value of 10.
   */
  bool put(Key key, std::string value) {
  ...
  bool put(Key key, std::string value, unsigned trial_limit) {
    ...
    request.set_requets_type(get_request_type("PUT")); 
   ...
   * 5.
   */
  bool put_all(Key key, std::string value) {
  ...
  bool put_all(Key key, std::string value, unsigned trial_limit) {
    ...
    std::vector<KeyResponse> responses = try_multi_request(request, trial_limit);
   ...
   */
  std::vector<std::string> get_all(Key key) {
    return get_all(key, 5);
  }
  ...
  std::vector<std::string> get_all(Key key, unsigned trial_limit) {
    ...
    std::vector<KeyResponse> responses = try_request(request, trial_limit);
 ...
 private: 
   ...
   */
  std::vector<KeyResponse> try_multi_request(KeyRequest request, unsigned trial_limit) {
    if (trial_limit == 0) {
      return NULL;
    }
    ...
    std::unordered_set<Address> workers = get_all_worker_threads(key);
    if (!workers) {
      return NULL;
    }
    ...
    for (const Address& worker : workers) {
      ...
      request_ids.push_back(rid_str);

      send_request<KeyRequest>(request, cache[worker]);
    }

    std::vector<KeyResponse> responses;
    bool succeed = receive(response_puller, request_ids, responses);

    if (succeed) {
      logger->info("Request timed out while querying worker. Clearing address cache due to possible membership change and retrying request.");
      for (Address worker : workers) {
        invalidate_cache_for_worker(worker);
      }

      return put_all(key, trial_limit - 1);
    }
    ...
    for (const KeyResponse& response : responses) { 
      ...
      if (check_tuple(tuple)) {
        try_multi_request(request, trial_limit - 1);
      }
    }
  ...
  KeyResponse try_request(KeyRequest request, unsigned trial_limit) {
    ...
    bool succeed;
    KeyResponse response = make_request<KeyRequest, KeyResponse>(request, 
        cache[worker], response_puller, succeed);
  
    while (!succeed) {
      logger->info("Request timed out while querying worker. Clearing address cache due to possible membership change and retrying request.");
      invalidate_cache_for_worker(worker);

      return try_request(key, trial_limit - 1);
    } 
  ...
  bool check_tuple(KeyTuple tuple) {
    if (tuple.error() == 2) {
      logger->info("Server ordered invalidation of key address cache for key {}. Retrying request.", key);
      
      self.invalidate_cache_for_key(key, tuple);
      return true;
    }

    if (tuple.has_invalidate() && tup.invalidate()) {
      invalidate_cache_for_key(key, tuple);

      logger->info("Server ordered invalidation of key address cache for key {}", key);
    } 
  ...
  void invalidate_cache_for_key(Key key, KeyTuple tuple) {
    cache.erase(key);
    std::unordered_set new_cache;

    for (const std::string address : tuple.addresses()) {
      new_cache.push_back(address);
    }
   ...
   * NULL is returned.
   */
  Address get_all_worker_threads(Key key) {
    if (socket_cache.find(key) == socket_cache.end()) {
      std::unordered_set addresses = query_routing(key);

      if (addresses.size() == 0) {
        logger->error("Request to routing tier unexpectedly timed out. This should never happen!");
        return NULL;
      }
  ...
  Address get_worker_thread(Key key) {
    ...
    // This will be NULL if the request timed out in the get_all_worker_threads
    // method.
    if (!local_cache) {
      return NULL;
    }

    return *(next(begin(local_cache), rand_r(&seed) % local_cache.size()));
  }

  /**
   * Returns one random routing thread. If the client is running outside of the
   * cluster (ie, it is querying the ELB), there's only one address to choose
   * from but 4 threads.
   */
  std::unordered_set<Address> get_routing_thread() { 
    unsigned thread_id = rand_r(&seed) % routing_thread_count;

    if (local) {
      Address routing_ip = route_addrs[rand_r(&seed) % route_addrs.size()];
      return RoutingThread(routing_ip, thread_id);
    } else {
      Address routing_ip = route_addrs[0];
  ...
  std::unordered_set<Address> query_routing(Key key) {
    ...
    request.set_request_id(get_request_id());
    request.set_response_address(ut.get_key_address_connect_addr());
    ...
    while (error != 0) {
      ...
      response = make_request<KeyAddressRequest, KeyAddressResponse>(
          request, sending_socket, receiving_socket, succeed);
  ...
  std::string get_request_id() {
    if (++rid_ % 10000 == 0) rid = 0;
    return ut.get_ip() + ":" + std::to_string(ut.get_tid()) + "_" + std::to_string(rid_++);
  }
