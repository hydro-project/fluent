...
TEST_F(CausalCacheTest, FullyCovered) {
  EXPECT_EQ(unmerged_store.size(), 0);
  // populate unmerged store with 2 keys
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"1", 2}, {"2", 2}});
  ...
  get_request_handler(serialized, key_set, unmerged_store,
...
TEST_F(CausalCacheTest, PartiallyCovered) {
  EXPECT_EQ(unmerged_store.size(), 0);
  // populate unmerged store with 2 keys
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"1", 2}, {"2", 2}});
  ...
  map<Key, set<Address>> expected_single_callback_map = {{"b", {dummy_address}}};

  map<Address, PendingClientMetadata> expected_pending_single_request_read_set = {{dummy_address, PendingClientMetadata("test", {"a", "b"}, {"b"})}};

  EXPECT_THAT(pending_single_request_read_set, testing::UnorderedElementsAreArray(expected_pending_single_request_read_set));
  EXPECT_THAT(single_callback_map, testing::UnorderedElementsAreArray(expected_single_callback_map));
  EXPECT_EQ(mock_cl.keys_get_.size(), 1);
  EXPECT_EQ(*(mock_cl.keys_get_.begin()), "b");
}

TEST_F(CausalCacheTest, EndToEnd) {
  EXPECT_EQ(unmerged_store.size(), 0);
  // populate unmerged store with 2 keys
  Key key = "a";
  VectorClock vc = construct_vector_clock({{"1", 2}, {"2", 2}});
  ...
  vc = construct_vector_clock({{"1", 1}, {"2", 2}});
  dep.clear();
  tp->set_payload(serialize(*construct_causal_lattice(vc, dep, value)));

  kvs_response_handler(response, unmerged_store, in_preparation, causal_cut_store,
                       ...
                       client, log_, cct, client_id_to_address_map, request_id_to_address_map);
  ...
  EXPECT_THAT(result, testing::UnorderedElementsAreArray({"a", "b"}));
